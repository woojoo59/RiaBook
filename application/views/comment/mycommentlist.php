			<tr class="commenttr">
					...
					<td class="commenttd2">
						...
						<div><a href="<?=base_url()?>comment/delete?index=<?=$_GET['index']?>&forcategory=<?=$_GET['forcategory']?>&cidx=<?=$cidx?>"><button class="commentbtn btn btn-outline-info">삭제</button></a></div>
					</td>
			</tr>
			<tr class="df none" id="recomment<?=$cidx?>">
				<td>
					<form method="post" action="<?=base_url()?>comment/commentwrite?index=<?=$_GET['index']?>&forcategory=<?=$_GET['forcategory']?>" class="commentform" id='commentform<?=$cidx?>'>
...
<script>
	...
	$('#recommentbtn<?=$cidx?>').on('click',()=>{
		if(recommentflag==0){
			...
			recommentflag = 1;
			flag<?=$cidx?> = 1;
			$('#commentform<?=$cidx?>').attr('action',"<?=base_url()?>comment/commentwrite?index=<?=$_GET['index']?>&forcategory=<?=$_GET['forcategory']?>")
		} else if (flag<?=$cidx?>==1){
	...
	$('#editbtn<?=$cidx?>').on('click',()=>{
		if(recommentflag==0){
			...
			recommentflag = 1;
			flag<?=$cidx?> = 1;
			$('#commentform<?=$cidx?>').attr('action','<?=base_url()?>comment/edit?index=<?=$_GET['index']?>&forcategory=<?=$_GET['forcategory']?>')
