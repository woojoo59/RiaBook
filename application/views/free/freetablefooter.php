	...
	<nav aria-label="Page navigation example" class="pagebtnsdiv">
	  <ul class="pagination pagination-lg pagebtns">
	  	<?php if($nowpage > 0){ ?>
	  		<li class="page-item">
	    	  <a class="page-link" href="<?=base_url()?>home/free?category=<?=$_GET['category']?>&sort=<?=$_GET['sort']?>" aria-label="Previous">
	  	...
	  	?>
	  		<li class="page-item">
	    	  <a class="page-link" href="<?=base_url()?>home/free?category=<?=$_GET['category']?>&sort=<?=$_GET['sort']?>&page=<?=$startpage-5?>" aria-label="Previous">
	    ...
	    ?>
	    	<li class="page-item <?php if($i==$nowpage){echo 'active';} ?>"><a class="page-link" href="<?=base_url()?>home/free?category=<?=$_GET['category']?>&sort=<?=$_GET['sort']?><?php if($i!=0){echo'&page='.$i;} ?>"><?=$i+1?></a></li>
	    <?php
	    	} 
	    ?>
	  	...
	  	?>
	    	<li class="page-item">
	    	  <a class="page-link" href="<?=base_url()?>home/free?category=<?=$_GET['category']?>&sort=<?=$_GET['sort']?>&page=<?=$endpage+1?>" aria-label="Next">
	  	...
	  	?>
	  	<?php if($nowpage < $pagecnt){ ?>
	  		<li class="page-item">
	    	  <a class="page-link" href="<?=base_url()?>home/free?category=<?=$_GET['category']?>&sort=<?=$_GET['sort']?>&page=<?=$pagecnt?>" aria-label="Next">
